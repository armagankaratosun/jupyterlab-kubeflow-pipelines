...
After editing Python files (like `jupyterlab_kubeflow_pipelines/routes.py`):

```bash
python -m py_compile jupyterlab_kubeflow_pipelines/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `jupyterlab_kubeflow_pipelines/*.py` files):
...
- Backend processing in `jupyterlab_kubeflow_pipelines/routes.py`
...
- **✅ Do**: `jupyterlab_kubeflow_pipelines/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'jupyterlab_kubeflow_pipelines:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyterlab_kubeflow_pipelines:open-panel';
  export const refreshData = 'jupyterlab_kubeflow_pipelines:refresh-data';
}
...
const plugin: JupyterFrontEndPlugin<void> = {
  id: 'jupyterlab_kubeflow_pipelines:plugin',
  autoStart: true,
...
- Backend routes: `jupyterlab_kubeflow_pipelines/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `jupyterlab_kubeflow_pipelines/` directory
...
1. **Read the backend first** — Check `jupyterlab_kubeflow_pipelines/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `jupyterlab_kubeflow_pipelines/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "jupyterlab_kubeflow_pipelines", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `jupyterlab_kubeflow_pipelines/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `jupyterlab_kubeflow_pipelines/`)
...
- **Output**: Creates files in `lib/` and `jupyterlab_kubeflow_pipelines/labextension/`
...
**After editing Python** (files in `jupyterlab_kubeflow_pipelines/`):
...
- Ask user to search for the extension ID (`jupyterlab_kubeflow_pipelines`) to see if it loaded
...
ls -la jupyterlab_kubeflow_pipelines/labextension/  # Should contain bundled extension
```
...
- Backend routes: `jupyterlab_kubeflow_pipelines/routes.py`
- Server extension config: `jupyter-config/server-config/jupyterlab_kubeflow_pipelines.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `jupyterlab_kubeflow_pipelines/routes.py`
...
```css
.jp-jupyterlab-kubeflow-pipelines-widget {
  padding: 8px;
}
```
...
**❌ Don't**: Use absolute imports like `from jupyterlab_kubeflow_pipelines.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'jupyterlab_kubeflow_pipelines:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyterlab_kubeflow_pipelines:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyterlab-kubeflow-pipelines-ClassName`
- **API routes** (in `jupyterlab_kubeflow_pipelines/routes.py`): `url_path_join(base_url, "jupyterlab_kubeflow_pipelines", "endpoint")`
